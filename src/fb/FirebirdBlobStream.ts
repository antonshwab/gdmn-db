...
export class FirebirdBlobStream {

    public connection: FirebirdConnection;
    public blobLink: FirebirdBlobLink;
    public handler?: Blob;
    ...
    public static async create(transaction: FirebirdTransaction): Promise<FirebirdBlobStream> {
        return await transaction.parent.context.statusAction(async (status) => {
            const blobId = new Uint8Array(8);
            const blobHandler = await transaction.parent.handler!.createBlobAsync(
                status, transaction.handler, blobId, 0, undefined);

            const blobLink = new FirebirdBlobLink(transaction.parent, blobId);

            return new FirebirdBlobStream(transaction.parent, blobLink, blobHandler);
        });
    }

    public static async open(transaction: FirebirdTransaction,
                             blobLink: FirebirdBlobLink): Promise<FirebirdBlobStream> {
        return await transaction.parent.context.statusAction(async (status) => {
            const blobHandler = await transaction.parent.handler!.openBlobAsync(
